...
# 10. Ana Grafik ve Analiz Güncelleme
@app.callback(
    [Output('scan-map-graph-3d', 'figure'), Output('scan-map-graph-2d', 'figure'), Output('polar-graph', 'figure'),
...
def update_all_graphs_and_analytics(points_json):
    ...
    if not points_json: return (empty_fig,) * 3 + ("Analiz için veri bekleniyor.", None) + ("--",) * 4
    df = pd.read_json(io.StringIO(points_json), orient='split')
    if df.empty: return (go.Figure(layout=dict(title='Henüz Nokta Verisi Yok...')),) * 3 + (
        "Analiz için veri bekleniyor.", None) + ("--",) * 4
    df_valid = df[(df['mesafe_cm'] > 0.1) & (df['mesafe_cm'] < 400.0)].copy();
    df_valid.dropna(subset=['x_cm', 'y_cm', 'z_cm'], inplace=True)
    fig_3d = go.Figure(layout=dict(title='3D Tarama Haritası', margin=dict(l=0, r=0, b=0, t=40)));
    ...
    if len(df_valid) > 3:
        ...
        fig_3d.update_layout(
            scene=dict(xaxis_title='Y Ekseni (cm)', yaxis_title='X Ekseni (cm)', zaxis_title='Z Ekseni (cm)',
                       aspectmode='data'))
        clustering_desc, df_clus = analyze_environment_shape(fig_2d, df_valid);
        ...
        try:
            ...
            if len(hull_points) >= 3: hull = ConvexHull(
                hull_points); area, perim = f"{hull.volume:.1f} cm²", f"{hull.area:.1f} cm"
        except Exception as e:
            print(f"ConvexHull hatası: {e}")
        width, depth = f"{df_valid['y_cm'].max() - df_valid['y_cm'].min():.1f} cm", f"{df_valid['x_cm'].max():.1f} cm"
        est_text = html.Div([html.P(estimate_geometric_shape(df_valid), className="fw-bold"), html.Hr(),
                             html.P(find_clearest_path(df_valid), className="fw-bold text-primary"), html.Hr(),
                             html.P(f"Kümeleme: {clustering_desc}")])
