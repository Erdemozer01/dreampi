...
visualization_tabs = dbc.Tabs([
    dbc.Tab([dbc.Row([dbc.Col(dcc.Dropdown(id='graph-selector-dropdown', options=[
        ...
        {'label': 'Zaman Serisi (Mesafe)', 'value': 'time'}], value='3d_map', clearable=False,
...
app.layout = html.Div(style={'padding': '20px'}, children=[
    navbar, dbc.Row(
        [dbc.Col([control_panel, html.Br(), stats_panel], md=4), dbc.Col([], md=8)]),
    dcc.Store(id='latest-scan-object-store'),
    dcc.Store(id='latest-scan-points-store'),
    dcc.Store(id='clustered-data-store'),
    dcc.Interval(id='interval-component-main', interval=2500,
                 n_intervals=0)
])
        ...
        with pd.ExcelWriter(buf, engine='xlsxwriter') as writer:
            ...
            if not points_df.empty:
                ...
                header_format = workbook.add_format(
                    {'bold': True, 'text_wrap': False, 'valign': 'vcenter', 'fg_color': '#4F81BD',
