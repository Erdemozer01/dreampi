...
def yorumla_model_secimi(selected_config_id):
    ...
    if not DJANGO_MODELS_AVAILABLE:
        return dbc.Alert("Veritabanı modellerine erişilemiyor.", color="danger")
    ...
    try:
        ...
        if "hata" not in analysis_result.lower():
            scan.ai_commentary = analysis_result
        ...
        return dbc.Alert(f"Analiz sırasında beklenmedik bir hata oluştu: {e}", color="danger")