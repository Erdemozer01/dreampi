...
@app.callback(
    ...
    [State("operation-mode", "value"),
     State("scan-angle-input", "value"),
     State("step-angle-input", "value"),
     State("buzzer-distance-input", "value"),
     State("fixed-tilt-angle-input", "value")]
)
...
def export_csv_callback(n_clicks, points_json):
    ...
    df = pd.read_json(io.StringIO(points_json), orient='split')
    return dcc.send_data_frame(df.to_csv, f"tarama_verisi.csv", index=False)


# 8. Verileri Excel dosyası olarak dışa aktarır
@app.callback(
...
def export_excel_callback(n_clicks, scan_json, points_json):
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        scan_info_df = pd.DataFrame([scan])
        scan_info_df.to_excel(writer, sheet_name='Tarama Bilgileri', index=False)
...
# 9. "Veri Tablosu" sekmesi aktif olduğunda tabloyu oluşturur
@app.callback(
...
def render_and_update_data_table(active_tab, points_json):
    ...
    return dash_table.DataTable(
        ...
        virtualization=True, fixed_rows={'headers': True},
...
# 10. ANA GRAFİK VE ANALİZ GÜNCELLEME FONKSİYONU
@app.callback(
...
def yorumla_model_secimi(selected_config_id, points_json):
    ...
    if not points_json:
        return dbc.Alert("Analiz edilecek veri bulunamadı.", color="warning"), None
