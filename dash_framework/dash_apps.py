...
@app.callback([Output('ai-yorum-sonucu', 'children'), Output('ai-image', 'children')],
...
def yorumla_model_secimi(selected_config_id):
    ...
    try:
        scan = get_latest_scan()
        if not scan: return [dbc.Alert("Analiz edilecek bir tarama bulunamadı.", color="warning"), None]
        ...
        if scan.ai_commentary and scan.ai_commentary.strip():
            ...
            prompt = "Bu 3D tarama verilerini analiz et. Ortamın genel şekli (oda, koridor vb.), belirgin nesneler, konumları ve şekilleri hakkında bilgi ver. z_cm (yükseklik) verisini dikkate al."
        ...
        if analysis_result_text and "hata" not in analysis_result_text.lower():
            try:
                ...
                image_prompt = f"Aşağıdaki metin analizini temel alarak, ortamın kuşbakışı şematik bir haritasını oluştur. Sadece resim çıktısı ver.\n\n--- ANALİZ ---\n{analysis_result_text}"
                generation_config = types.GenerationConfig(response_mime_types=['image/png'])
                image_response = image_model.generate_content(contents=image_prompt,
                                                              generation_config=generation_config)

                if image_response.parts and hasattr(image_response.parts[0], 'inline_data') and image_response.parts[
                    0].inline_data.data:
                    part = image_response.parts[0]
                    b64_img = base64.b64encode(part.inline_data.data).decode('utf-8')
                    image_component = html.Img(src=f"data:{part.inline_data.mime_type};base64,{b64_img}",
