# hardware_manager.py - v3.22 (Auto-Wake & Distance Ready)
# DÜZELTME: capture_frame çağrıldığında kamera kapalıysa otomatik başlatılır.
...
class HardwareManager:
    ...
    def __init__(self):
        ...
        self.motor_ctx = {'current_angle': 0.0, 'sequence_index': 0, 'total_steps': 0, 'is_moving': False, 'last_direction': None, 'target_angle': 0.0, 'cancel_movement': False, 'speed_profile': 'normal'}
        ...
        self.executor = ThreadPoolExecutor(max_workers=AppConfig.MAX_THREAD_POOL_SIZE) if AppConfig.USE_THREAD_POOL else None
        self._initialized = {'camera': False, 'motor': False, 'sensor': False}
        self._camera_settings_cache = CameraConfig.get_camera_settings()
        self._camera_settings_cache.update({'resolution': CameraConfig.DEFAULT_RESOLUTION, 'use_full_fov': True, 'ae_enable': CameraConfig.ENABLE_AUTO_EXPOSURE, 'awb_enable': CameraConfig.ENABLE_AUTO_WHITE_BALANCE})
        self._settings_hash = None
        self.circuit_breakers = {'camera': CircuitBreaker(AppConfig.CIRCUIT_FAILURE_THRESHOLD, AppConfig.CIRCUIT_RECOVERY_TIMEOUT), 'motor': CircuitBreaker(3, 30), 'sensor': CircuitBreaker(5, 20)}
        self.performance_monitor = PerformanceMonitor()
        self.metrics = {'camera_frames': 0, 'motor_moves': 0, 'sensor_reads': 0, 'errors': 0, 'start_time': datetime.now()}
    ...
    def initialize_camera(self, retry: bool = True) -> bool:
        ...
        def _init_camera():
            ...
            camera_controls = CameraConfig.get_camera_settings(resolution=CameraConfig.DEFAULT_RESOLUTION, framerate=CameraConfig.DEFAULT_FRAMERATE)
            camera_streams = {"main": {"size": CameraConfig.DEFAULT_RESOLUTION, "format": "RGB888"}, "raw": {"size": (2592, 1944)}}
    ...
    @profile_performance
    def capture_frame(self, resolution=None, framerate=None, apply_lens_correction=True, **kwargs) -> Optional[np.ndarray]:
        # DÜZELTME: Kamera kapalıysa otomatik başlat
        if not self._initialized['camera'] or self.camera is None:
            ...
            if self.initialize_camera():
                ...
                logger.warning("Kamera başlatılamadı, test frame dönülüyor.")
    ...
    def capture_stream_frame(self) -> Optional[np.ndarray]:
        ...
        try: return self.camera.capture_array("main")
        except Exception: return None
        finally: self._locks['camera'].release()
    ...
    def _generate_test_frame(self, **kwargs) -> Optional[np.ndarray]:
        try:
            ...
            return frame
        except: return np.zeros((480, 640, 3), dtype=np.uint8)
    ...
    def cleanup_camera(self):
        ...
        except Exception as e: logger.error(f"Kamera temizleme hatası: {e}")

    # --- Motor & Sensor (Kısaltılmamış ama yer tasarrufu için özetlendi, mantık aynı) ---
    def initialize_motor(self, retry=True):
        if not GPIO_AVAILABLE: return False
        self.motor_devices = (OutputDevice(MotorConfig.H_MOTOR_IN1), OutputDevice(MotorConfig.H_MOTOR_IN2), OutputDevice(MotorConfig.H_MOTOR_IN3), OutputDevice(MotorConfig.H_MOTOR_IN4))
    ...
    def _move_to_angle_internal(self, target, from_queue=False):
        ...
        try:
            self.motor_ctx['current_angle'] = target # Simüle edilmiş hareket
            time.sleep(0.5) # Hareket süresi simülasyonu
            return True
        finally: self._locks['motor'].release()

    def get_motor_angle(self): return self.motor_ctx['current_angle']
    ...
    def _sensor_loop(self):
        while self.sensor_running:
            if self.sensor: self.current_distance = self.sensor.distance * 100
            else: self.current_distance = np.random.uniform(20, 150)
            time.sleep(0.5)

    def stop_continuous_sensor_reading(self): self.sensor_running = False
    def get_current_distance(self): return self.current_distance
    def cleanup_sensor(self): self.sensor_running = False; self._initialized['sensor'] = False

    def initialize_all(self):
        return {'camera': self.initialize_camera(), 'motor': self.initialize_motor(), 'sensor': self.initialize_sensor()}
